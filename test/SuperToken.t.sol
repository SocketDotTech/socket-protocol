...
contract SuperTokenTest is AuctionHouseTest {
    ...
    function createDeployPayloadDetailsArray(uint32 chainSlug_) internal returns (PayloadDetails[] memory) {
        ...
        payloadDetails[0] = createDeployPayloadDetail(
            ...
            appContracts.superTokenDeployer.creationCodeWithArgs(appContracts.superToken)
        );
        payloadDetails[1] = createDeployPayloadDetail(
            chainSlug_,
            address(appContracts.superTokenDeployer),
            appContracts.superTokenDeployer.creationCodeWithArgs(appContracts.limitHook)
        );
    ...
    function createConfigurePayloadDetailsArray(uint32 chainSlug_) internal returns (PayloadDetails[] memory) {
        address superTokenForwarder =
            appContracts.superTokenDeployer.forwarderAddresses(appContracts.superToken, chainSlug_);
        address limitHookForwarder =
            appContracts.superTokenDeployer.forwarderAddresses(appContracts.limitHook, chainSlug_);
    ...
    function createBridgePayloadDetailsArray(uint32 srcChainSlug_, uint32 dstChainSlug_)
        internal
        returns (PayloadDetails[] memory)
    {
        PayloadDetails[] memory payloadDetails = new PayloadDetails[](4);

        address deployedSrcToken = IForwarder(userOrder.srcToken).getOnChainAddress();
        address deployedDstToken = IForwarder(userOrder.dstToken).getOnChainAddress();
        ...
        payloadDetails[0] = createExecutePayloadDetail(
            ...
            userOrder.srcToken,
            abi.encodeWithSignature("lockTokens(address,uint256)", userOrder.user, userOrder.srcAmount)
        );
        ...
        payloadDetails[2] = createExecutePayloadDetail(
            ...
            address(appContracts.superTokenApp),
            userOrder.dstToken,
            abi.encodeWithSignature("mint(address,uint256)", userOrder.user, userOrder.srcAmount)
        );
        ...
        payloadDetails[3] = createExecutePayloadDetail(
            ...
            address(appContracts.superTokenApp),
            userOrder.srcToken,
            abi.encodeWithSignature("burn(address,uint256)", userOrder.user, userOrder.srcAmount)
        );
    ...
    function createCancelPayloadDetailsArray(uint32 srcChainSlug_) internal returns (PayloadDetails[] memory) {
        PayloadDetails[] memory payloadDetails = new PayloadDetails[](1);
        ...
        payloadDetails[0] = createExecutePayloadDetail(
            ...
            address(appContracts.superTokenApp),
            userOrder.srcToken,
            abi.encodeWithSignature("unlockTokens(address,uint256)", userOrder.user, userOrder.srcAmount)
        );
    ...
    function testContractDeployment() public {
        ...
        _deploy(payloadIds, arbChainSlug, maxFees, appContracts.superTokenDeployer, payloadDetails);
    }
    ...
    function testConfigure() public {
        ...
        _configure(payloadIds, address(appContracts.superTokenApp), maxFees, payloadDetails);
    }
    ...
    function beforeBridge() internal {
        ...
        _deploy(payloadIds, optChainSlug, maxFees, appContracts.superTokenDeployer, payloadDetails);

        payloadIds = getWritePayloadIds(optChainSlug, getPayloadDeliveryPlug(optChainSlug), 1);
        payloadDetails = createConfigurePayloadDetailsArray(optChainSlug);
        _configure(payloadIds, address(appContracts.superTokenApp), maxFees, payloadDetails);

        payloadIds = getWritePayloadIds(arbChainSlug, getPayloadDeliveryPlug(arbChainSlug), 2);
    ...
    function _bridge() internal returns (bytes32, bytes32[] memory, PayloadDetails[] memory) {
        ...
        payloadIds[0] = getWritePayloadId(
            srcChainSlug, address(getSocketConfig(srcChainSlug).payloadDeliveryPlug), writePayloadIdCounter++
        );
        payloadIds[1] = bytes32(readPayloadIdCounter++);
        payloadIds[2] = getWritePayloadId(
            dstChainSlug, address(getSocketConfig(dstChainSlug).payloadDeliveryPlug), writePayloadIdCounter++
        );
        ...
        PayloadDetails[] memory payloadDetails = createBridgePayloadDetailsArray(srcChainSlug, dstChainSlug);
        ...
        bidAndValidate(maxFees, bridgeAsyncId, address(appContracts.superTokenApp), payloadDetails);
    ...
    function testBridge() public {
        ...
        finalizeAndExecute(bridgeAsyncId, payloadIds[2], false, payloadDetails[2]);
        finalizeAndExecute(bridgeAsyncId, payloadIds[3], false, payloadDetails[3]);
    }
    ...
    function testWithdrawTo() public {
        ...
        socketConfig.payloadDeliveryPlug.deposit{value: depositAmount}(token, depositAmount, appGateway);
        assertEq(
            depositAmount, socketConfig.payloadDeliveryPlug.balanceOf(appGateway, token), "Balance should be correct"
        );

        appContracts.superTokenApp.withdrawFeeTokens(chainSlug, token, withdrawAmount, receiver);

        bytes32[] memory withdrawPayloadIds = new bytes32[](1);
        withdrawPayloadIds[0] = getWritePayloadId(
            chainSlug, address(getSocketConfig(chainSlug).payloadDeliveryPlug), writePayloadIdCounter++
        );
        bytes32 withdrawAsyncId = getCurrentAsyncId();
        asyncCounterTest++;

        bytes memory withdrawPayload = abi.encode(WITHDRAW, abi.encode(appGateway, token, withdrawAmount, receiver));
        ...
        bidAndValidate(maxFees, withdrawAsyncId, address(appContracts.superTokenApp), payloadDetails);
        finalizeAndExecute(withdrawAsyncId, withdrawPayloadIds[0], true, payloadDetails[0]);
    }
}
