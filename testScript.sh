...
source .env && cast send $DEPLOYER "deployMultiChainContracts(uint32[])" '[421614, 11155420]' --rpc-url $ETH_RPC_URL --private-key $PRIVATE_KEY
...
## set limits for the app gateway using API
source .env && cast send $DEPLOYER "deployContracts(uint32)" 421614 --rpc-url $ETH_RPC_URL --private-key $PRIVATE_KEY
source .env && cast send $APP_GATEWAY "incrementCounters(address[])" '[0x4507f726d8ca980e3a1800a8d972792d7ff46f65]' --rpc-url $ETH_RPC_URL --private-key $PRIVATE_KEY
...
## Counter Inbox
forge script script/counter-inbox/DeployCounterAndGateway.s.sol --broadcast --skip-simulation
...
forge script script/counter-inbox/CheckGatewayCounter.s.sol --broadcast --skip-simulation
...
## Check Limits
forge script script/admin/checkLimits.s.sol --broadcast --skip-simulation
forge script script/admin/UpdateLimits.s.sol --broadcast --skip-simulation


# add fees
source .env && forge script script/DepositFees.s.sol --broadcast --skip-simulation --private-key $PRIVATE_KEY
