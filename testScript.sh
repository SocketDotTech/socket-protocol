...
cast send $APP_GATEWAY "setTimeout(uint256)" 0 --private-key $PRIVATE_KEY
...
source .env && cast send $COUNTER_INBOX "connectSocket(address,address,address)" $APP_GATEWAY $SOCKET $SWITCHBOARD --private-key $SPONSOR_KEY 
source .env && cast send $COUNTER_INBOX "increaseOnGateway(uint256)" 100 --private-key $SPONSOR_KEY 
