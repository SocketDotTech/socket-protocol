...
async function main() {
    ...
    const addressesPath = path.join(__dirname, '../../deployments/dev_addresses.json');
    ...
    for (const contractName of upgradeableContracts) {
        ...
        try {
            ...
            console.log(`Current implementation address for ${contractName}: ${currentImplAddress}`);

            // Get the implementation address from JSON
            const newImplementation = addresses[OFF_CHAIN_VM_CHAIN_ID][`${contractName}Impl`];
            ...
            const updatedImplAddress = await upgrades.erc1967.getImplementationAddress(PROXY_ADDRESS);
            ...
            if (error.message.includes("doesn't look like an ERC 1967 proxy")) {
                console.log(`${contractName} at ${PROXY_ADDRESS} is not a proxy contract, skipping...`);
                continue;
            }
            throw error;
        }
