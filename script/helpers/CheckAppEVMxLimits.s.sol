...
contract CheckLimitsScript is Script {
    function run() external {
        ...
        LimitParams memory scheduleLimit = watcherContract.watcherPrecompileLimits__().getLimitParams(SCHEDULE, appGateway);
        LimitParams memory queryLimit = watcherContract.watcherPrecompileLimits__().getLimitParams(QUERY, appGateway);
        LimitParams memory finalizeLimit = watcherContract.watcherPrecompileLimits__().getLimitParams(FINALIZE, appGateway);
